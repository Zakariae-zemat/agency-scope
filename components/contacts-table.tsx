            ...
            <div className="md:hidden p-4 space-y-3">
              {contacts.length === 0 ? (
                ...
                contacts.map((contact) => {
                  ...
                  return (
                    <div key={contact.id} className="bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 rounded-xl p-4 space-y-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            ...
                            {isViewed && (
                              <CheckCircle2 className="h-4 w-4 text-slate-600" strokeWidth={3} />
                            )}
                          </div>
                          {contact.title && (
                            <span className="inline-block px-2 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-bold">
                              {contact.title}
                            </span>
                          )}
                        </div>
                        <button
                          ...
                          className={`p-2 rounded-lg font-bold transition-all ${
                            isViewed 
                              ? 'border-2 border-slate-200 bg-white text-slate-900 hover:border-slate-900' 
                      ...
                      <div className="space-y-2 text-sm">
                        {contact.agency && (
                          <div>
                            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Agency</div>
                            <div className="font-bold text-slate-900 dark:text-slate-100">{contact.agency.name}</div>
                            {contact.agency.state && (
                              <div className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{contact.agency.state}</div>
                            )}
                          </div>
                        )}
                        {contact.department && (
                          <div>
                            <div className="text-xs text-slate-500 dark:text-slate-400 font-medium mb-1">Department</div>
                            <div className="text-slate-700 dark:text-slate-300">{contact.department}</div>
                          </div>
                        )}
                      </div>
                    </div>
